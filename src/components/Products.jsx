...
const Products = () => {
  ...
  useEffect(() => {
    if (data) {
      const prods = data.products.edges.map(({ node }) => {
        ...
        return [node.id, node.title, node.description, price, node.status]
      });
      setProducts(prods);
      console.log("🚀 ~ file: Products.jsx ~ line 34 ~ useEffect ~ prods", data)
    }
  }, [data]);
  ...
  return (
    <Card title="Product List" sectioned>
      {error &&
        <Heading element="h4" style={{ color: 'red' }}>Error :(</Heading>
      }
